...
enum TaskPriority: String, Codable, CaseIterable, Identifiable, Hashable {
    ...
    /// Compact chip / status label (Todoist P1–P3 style).
...
final class TaskListEntity {
    ...
    @Relationship(deleteRule: .cascade, inverse: \PlannerNoteEntity.list)
    var notes: [PlannerNoteEntity]? = []
...
/// Rich note document living inside a task list (Apple Notes folder model).
@Model
final class PlannerNoteEntity {
    var id: UUID = UUID()
    var title: String = ""
    /// Plain text body used for previews and lightweight editing.
    var body: String = ""
    /// Optional RTF blob for bold / italic / headings (NSAttributedString).
    var bodyRTF: Data = Data()
    var isPinned: Bool = false
    var createdAt: Date = Date()
    var updatedAt: Date = Date()
    var list: TaskListEntity?

    init(title: String = "", body: String = "", list: TaskListEntity? = nil) {
        id = UUID()
        self.title = title
        self.body = body
        bodyRTF = Data()
...
enum PlannerStore {
    ...
    @discardableResult
    static func addNote(
        to list: TaskListEntity?,
        title: String = "",
        body: String = "",
        in context: ModelContext
    ) -> PlannerNoteEntity {
        let resolved = list ?? ensureInbox(in: context)
        let note = PlannerNoteEntity(title: title, body: body, list: resolved)
        context.insert(note)
        try? context.save()
        return note
    }
    ...
    @MainActor
    static func deleteNote(_ note: PlannerNoteEntity, in context: ModelContext) {
        context.delete(note)
        try? context.save()
    }

    @MainActor
    static func touchNote(_ note: PlannerNoteEntity, in context: ModelContext) {
        note.updatedAt = .now
        try? context.save()
    }
}
