...
final class SpendTransactionEntity {
    ...
    /// Plaid `account_id` when synced (empty for manual / demo).
    var accountID: String = ""
    /// Plaid `item_id` for the linked institution (empty for manual / demo).
    var enrollmentID: String = ""
    /// Denormalized institution label for multi-bank lists (e.g. "Chase").
    var institutionName: String = ""
    var merchant: String = ""
    var amount: Double = 0
    ...
    /// "Chase · Checking ••1234" when both are known.
    var sourceCaption: String {
        let parts = [institutionName, accountName]
