...
enum SpendStore {
    ...
    static func syncEnrollment(
        ...
        var cursor = enrollment.syncCursor.isEmpty
            ...
            : enrollment.syncCursor
        if forceFullResync || (!cursor.isEmpty && !hasLocalPlaidTransactions(for: enrollment.enrollmentID, in: context)) {
            cursor = ""
            enrollment.syncCursor = ""
    ...
    /// True when SwiftData already holds at least one non-demo transaction for this Plaid Item.
    private static func hasLocalPlaidTransactions(for enrollmentID: String, in context: ModelContext) -> Bool {
        let rows = (try? context.fetch(FetchDescriptor<SpendTransactionEntity>())) ?? []
        return rows.contains {
            !$0.remoteID.hasPrefix("demo-")
                && !$0.remoteID.hasPrefix("manual-")
    ...
    static func localPlaidTransactionCount(in context: ModelContext) -> Int {
        let rows = (try? context.fetch(FetchDescriptor<SpendTransactionEntity>())) ?? []
        return rows.filter { !$0.remoteID.hasPrefix("demo-") && !$0.remoteID.hasPrefix("manual-") }.count
    }

    static func transactionCount(forEnrollmentID enrollmentID: String, in context: ModelContext) -> Int {
        let rows = (try? context.fetch(FetchDescriptor<SpendTransactionEntity>())) ?? []
        return rows.filter { $0.enrollmentID == enrollmentID }.count
    }

    struct SyncAllResult {
        var upserted: Int
        var failures: [(institution: String, message: String)]

        var summary: String {
            if failures.isEmpty {
                return upserted == 0
                    ? "Already up to date."
                    : "Updated \(upserted) transaction\(upserted == 1 ? "" : "s")."
            }
            let names = failures.map(\.institution).joined(separator: ", ")
            if upserted == 0 {
                return "Couldn’t sync \(names)."
            }
            return "Updated \(upserted); failed: \(names)."
        }
    }
    ...
    static func syncAllEnrollments(
        ...
        for enrollment in enrollments {
            do {
                ...
                let name = enrollment.institutionName.isEmpty ? "Bank" : enrollment.institutionName
                failures.append((name, error.localizedDescription))
            }
        }
        if total == 0, !failures.isEmpty, failures.count == enrollments.count {
            throw NSError(
                domain: "SpendStore",
                code: 1,
                userInfo: [NSLocalizedDescriptionKey: failures.map { "\($0.institution): \($0.message)" }.joined(separator: " · ")]
            )
        }
        return SyncAllResult(upserted: total, failures: failures)
    }
    ...
    static func upsertTransactions(
        ...
        for dto in remote {
            ...
            let accountID = dto.account_id ?? ""
            let accountName = accountNames[accountID] ?? enrollmentFallbackAccountName(dto.account_id)
            ...
            if let row = byRemote[dto.id] {
                ...
                row.institutionName = institution
    ...
    static func disconnectEnrollment(_ enrollment: SpendEnrollmentEntity, in context: ModelContext) {
        let itemID = enrollment.enrollmentID
        KeychainStore.deletePlaidAccessToken(itemID: itemID)

        let txs = (try? context.fetch(FetchDescriptor<SpendTransactionEntity>())) ?? []
        let removedRemoteIDs = Set(
            txs.filter { $0.enrollmentID == itemID }.map(\.remoteID)
        )
        for tx in txs where tx.enrollmentID == itemID {
            context.delete(tx)
        }

        if !removedRemoteIDs.isEmpty {
            let tracked = (try? context.fetch(FetchDescriptor<SpendTrackedItemEntity>())) ?? []
            let logs = (try? context.fetch(FetchDescriptor<SpendUseLogEntity>())) ?? []
            for item in tracked where removedRemoteIDs.contains(item.linkedTransactionRemoteID) {
                let trackedID = item.id
                for log in logs where log.itemID == trackedID {
                    context.delete(log)
                }
                context.delete(item)
            }
        ...
        CadenceCloudStore.save(context, label: "disconnectEnrollment")
    }
}
