...
struct WheelNav: View {
    ...
    private var expandSpring: Animation {
        // Match RootView.appMenuSpring so dial + sheet don't fight.
    ...
    var body: some View {
        ...
        dialStack
            ...
            .accessibilityHint(
                endStops
                    ? "Tap an app to switch. Drag sideways to scroll. Stops at the first and last app. Swipe up to expand the menu."
                    : "Tap an app to switch. Drag sideways to spin. Flick for momentum. Swipe up to expand the menu."
            )
    ...
    private var dialGesture: some Gesture {
        DragGesture(minimumDistance: 0, coordinateSpace: .local)
            ...
            .onEnded { value in
                ...
                case .undecided:
                    // Idle dock: tap only switches left/right — don't open the arc scroll UI.
                    // Swipe (horizontal) still wakes via onChanged once a real drag starts.
                    if idleAmount < 0.5 {
                        wake(animated: false)
                    }
...
struct WheelAppMenuOverlay: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            Group {
                if isEditing {
                    ...
                    appMenuBody
                        // Don't let the open spring re-animate each icon/label separately.
        ...
        // Keep icons/labels glued to the sheet during the open spring (no staggered fades).
        .geometryGroup()
        .accessibilityElement(children: .contain)
    ...
    private func appGrid(_ gridItems: [WheelNavItem], mode: GridMode) -> some View {
        let columns = 4
        let rowCount = max(1, Int(ceil(Double(gridItems.count) / Double(columns))))
                ...
                HStack(spacing: 14) {
                    ForEach(0..<columns, id: \.self) { col in
                        ...
                        if index < gridItems.count {
                            cell(gridItems[index], sort: Double(index), mode: mode)
                                .frame(maxWidth: .infinity)
                        } else {
                            Color.clear
                                .frame(maxWidth: .infinity)
        ...
        .padding(.horizontal, Theme.Space.xl)
        .animation(isEditing ? .spring(response: 0.32, dampingFraction: 0.86) : nil, value: gridItems.map(\.id))
    }
