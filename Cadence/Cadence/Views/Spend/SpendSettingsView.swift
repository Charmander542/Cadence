...
struct SpendSettingsView: View {
    ...
    @State private var enrollmentPendingDisconnect: SpendEnrollmentEntity?
    ...
    var body: some View {
        Form {
            ...
            Section {
                Button {
                    ...
                    settingsCTALabel(
                        linkCoordinator.isPreparing
                            ? "Preparing Link…"
                            : (enrollments.isEmpty ? "Connect bank or card" : "Connect another bank"),
                ...
                settingsDetailIntro("Link as many banks or cards as you need (Plaid Trial Items, max 10). Each connection syncs on its own.")
            }
            ...
            Section {
                if enrollments.isEmpty {
                    ...
                    ForEach(enrollments, id: \.id) { enrollment in
                        let purchaseCount = SpendStore.transactionCount(
                            forEnrollmentID: enrollment.enrollmentID,
                            in: modelContext
                        )
                        VStack(alignment: .leading, spacing: 4) {
                            LabeledContent(enrollment.institutionName.isEmpty ? "Bank" : enrollment.institutionName) {
                                Text(enrollment.isSandbox ? "Sandbox" : "Live")
                                    .foregroundStyle(Theme.muted)
                            }
                            Text("\(purchaseCount) purchase\(purchaseCount == 1 ? "" : "s") stored")
                                .font(.caption2)
                                .foregroundStyle(Theme.muted)
                        ...
                        .swipeActions(edge: .trailing, allowsFullSwipe: false) {
                            Button(role: .destructive) {
                                enrollmentPendingDisconnect = enrollment
                            } label: {
                                Label("Remove", systemImage: "trash")
                            }
                        }
                        .accessibilityHint("Swipe to disconnect \(enrollment.institutionName)")
                    }
                }
            } header: {
                settingsDetailSectionHeader("Connections")
            } footer: {
                settingsDetailIntro("Removing a bank deletes its imported purchases from Spend (cost/use trackers for those buys go too). Other banks stay linked.")
            }
        ...
        .confirmationDialog(
            disconnectTitle,
            isPresented: Binding(
                get: { enrollmentPendingDisconnect != nil },
                set: { if !$0 { enrollmentPendingDisconnect = nil } }
            ),
            titleVisibility: .visible
        ) {
            if let enrollment = enrollmentPendingDisconnect {
                let count = SpendStore.transactionCount(
                    forEnrollmentID: enrollment.enrollmentID,
                    in: modelContext
                )
                Button("Remove \(enrollment.institutionName)", role: .destructive) {
                    SpendStore.disconnectEnrollment(enrollment, in: modelContext)
                    enrollmentPendingDisconnect = nil
                    showStatus(
                        count == 0
                            ? "Disconnected \(enrollment.institutionName)."
                            : "Disconnected \(enrollment.institutionName) and removed \(count) purchases.",
                        tone: .neutral
                    )
                }
            ...
            if let enrollment = enrollmentPendingDisconnect {
                ...
                Text(count == 0
                     ? "This unlinks the bank. Other connections stay."
                     : "This unlinks the bank and deletes \(count) imported purchase\(count == 1 ? "" : "s"). Other banks stay.")
            }
        }
    ...
    private var disconnectTitle: String {
        guard let enrollment = enrollmentPendingDisconnect else { return "Remove bank?" }
        let name = enrollment.institutionName.isEmpty ? "this bank" : enrollment.institutionName
        return "Remove \(name)?"
    }
    ...
    private func syncAll() async {
        ...
        defer { isBusy = false }
        do {
            let result = try await SpendStore.syncAllEnrollments(in: modelContext)
            showStatus(
                result.summary,
                tone: result.failures.isEmpty ? .accent : .danger,
                icon: result.failures.isEmpty ? "checkmark.circle.fill" : "exclamationmark.triangle.fill"
            )
        } catch {
            showStatus(error.localizedDescription, tone: .danger, icon: "exclamationmark.triangle.fill")
        }
