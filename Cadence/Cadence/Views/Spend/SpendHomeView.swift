...
struct SpendHomeView: View {
    ...
    /// When multiple banks are linked, optional Plaid item_id filter (`nil` = all).
    @State private var enrollmentFilterID: String?
    ...
    private var scopedTransactions: [SpendTransactionEntity] {
        guard let enrollmentFilterID else { return Array(transactions) }
        return transactions.filter { $0.enrollmentID == enrollmentFilterID }
    }

    private var sortedEnrollments: [SpendEnrollmentEntity] {
        enrollments.sorted {
            $0.institutionName.localizedCaseInsensitiveCompare($1.institutionName) == .orderedAscending
        }
    }

    private var linkedBanksSubtitle: String {
        let names = sortedEnrollments
            .map(\.institutionName)
            .map { $0.trimmingCharacters(in: .whitespacesAndNewlines) }
            .filter { !$0.isEmpty }
        if names.isEmpty { return "Plaid connection" }
    ...
    private var visibleTransactions: [SpendTransactionEntity] {
        let txs = SpendStore.spendingTransactions(scopedTransactions, in: monthRange, category: categoryFilter)
        return txs.sorted { $0.postedAt > $1.postedAt }
    }

    var body: some View {
        mainChrome
            .onChange(of: enrollments.count) { _, _ in
                clearStaleEnrollmentFilter()
            }
    ...
    private var spendHomeSheets: some ViewModifier {
        SpendHomeSheetsModifier(
            selectedTransaction: $selectedTransaction,
            selectedItem: $selectedItem,
            selectedCategory: $selectedCategory,
            selectedUserCategory: $selectedUserCategory,
            showAddPurchase: $showAddPurchase,
            showAddTracked: $showAddTracked,
    ...
    private func clearStaleEnrollmentFilter() {
        guard let filter = enrollmentFilterID else { return }
        if !enrollments.contains(where: { $0.enrollmentID == filter }) {
            enrollmentFilterID = nil
        }
    }
        ...
        Theme.Card {
            HStack(alignment: .top, spacing: Theme.Space.md) {
                ...
                VStack(alignment: .leading, spacing: 4) {
                    Text(connected
                         ? (count == 1 ? "Bank linked" : "\(count) banks linked")
                         : "Link purchases")
                        .font(.subheadline.weight(.semibold))
                        .foregroundStyle(Theme.ink)
                    Text(connected ? linkedBanksSubtitle : "Connect with Plaid to import and categorize buys.")
                        .font(.caption)
                        .foregroundStyle(Theme.muted)
                        .fixedSize(horizontal: false, vertical: true)
                    if connected, count > 1 {
                        Text("Filter purchases below by bank, or sync all at once.")
                            .font(.caption2)
                ...
                if connected {
                    ...
                    .buttonStyle(.plain)
                    .disabled(isSyncing)
                    .accessibilityHint("Syncs purchases from all linked banks")
                }
                Button {
                    showConnectInfo = true
                } label: {
            ...
            if connected, count > 1 {
                ScrollView(.horizontal, showsIndicators: false) {
                    HStack(spacing: Theme.Space.sm) {
                        bankChip(title: "All", selected: enrollmentFilterID == nil) {
                            enrollmentFilterID = nil
                        }
                        ForEach(sortedEnrollments, id: \.id) { enrollment in
                            bankChip(
                                title: enrollment.institutionName.isEmpty ? "Bank" : enrollment.institutionName,
                                selected: enrollmentFilterID == enrollment.enrollmentID
                            ) {
                                enrollmentFilterID = enrollment.enrollmentID
                            }
                        }
                    }
                }
    ...
    private func bankChip(title: String, selected: Bool, action: @escaping () -> Void) -> some View {
        Button(action: action) {
            Text(title)
                ...
                .overlay(
                    Capsule(style: .continuous)
                        .strokeBorder(selected ? Theme.accent.opacity(0.7) : Theme.hairline, lineWidth: 1)
                )
        }
        .buttonStyle(.plain)
        .accessibilityAddTraits(selected ? .isSelected : [])
        .accessibilityHint("Filters Spend to \(title)")
    }
    ...
    private func transactionRow(_ tx: SpendTransactionEntity) -> some View {
        ...
        return HStack(spacing: Theme.Space.sm + 2) {
            ...
            VStack(alignment: .leading, spacing: 1) {
                ...
                Text(sourceLine(for: tx, subcategoryName: subName))
                    .font(.caption2)
                    .foregroundStyle(Theme.muted)
                    .lineLimit(1)
            }
        ...
        .accessibilityLabel({
            ...
            if !tx.sourceCaption.isEmpty { parts.append(tx.sourceCaption) }
    ...
    private func sourceLine(for tx: SpendTransactionEntity, subcategoryName: String?) -> String {
        var parts = [shortDate(tx.postedAt), subcategoryName ?? tx.category.title]
        if !tx.sourceCaption.isEmpty {
            parts.append(tx.sourceCaption)
        }
        return parts.joined(separator: " · ")
    }
    ...
    private func syncNow() async {
        ...
        do {
            let result = try await SpendStore.syncAllEnrollments(in: modelContext, forceFullResync: true)
            SpendStore.refreshAutoCategories(in: modelContext)
            let total = SpendStore.localPlaidTransactionCount(in: modelContext)
            if total == 0 {
                syncMessage = result.upserted == 0
                    ? "No purchases yet — try again in a minute while Plaid loads history"
                    : "Synced, waiting on bank history…"
            } else if result.failures.isEmpty {
                syncMessage = "Loaded \(total) purchase\(total == 1 ? "" : "s") from \(enrollments.count) bank\(enrollments.count == 1 ? "" : "s")"
...
/// Sheets extracted so `SpendHomeView.body` type-checks in reasonable time.
private struct SpendHomeSheetsModifier: ViewModifier {
    @Binding var selectedTransaction: SpendTransactionEntity?
    @Binding var selectedItem: SpendTrackedItemEntity?
    @Binding var selectedCategory: SpendCategory?
    @Binding var selectedUserCategory: UserCategorySheetItem?
    @Binding var showAddPurchase: Bool
    @Binding var showAddTracked: Bool
    @Binding var showBudgets: Bool
    @Binding var showCategories: Bool
    @Binding var showConnectInfo: Bool
        ...
        NavigationStack {
            List {
                Section {
                    ...
                    LabeledContent("Account", value: {
                        let caption = transaction.sourceCaption
                        if !caption.isEmpty { return caption }
                        return transaction.accountName.isEmpty ? "—" : transaction.accountName
                    }())
                } header: {
